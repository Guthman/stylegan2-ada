...
class HDF5Exporter:
    ...
    def add_images(self, img):
        ...
        if self.print_progress and (self.cur_images - 1) % self.progress_interval >= self.progress_interval - img.shape[
            0]:
            print('%d / %d\r' % (self.cur_images, self.expected_images), end='', flush=True)
...
def info(tfrecord_dir):
    ...
    print(
        '%-20s%s' % ('Label range:', '%g -- %g' % (label_min, label_max) if num_images and dset.label_size else 'n/a'))
...
def execute_cmdline(argv):
    ...
    p = add_command('create_from_images_with_labels',
                    'Create dataset from a directory full of images and class labels.',
                    'create_from_images datasets/mydataset myimagedir')
    p.add_argument('tfrecord_dir', help='New dataset directory to be created')
    p.add_argument('image_dir', help='Directory containing the images')
    ...
    p.add_argument('--num_validation', help='Number of images to pick for the validation set (default: 0 = no images)',
                   type=int, default=0)
    p.add_argument('--mirror', help='Number of images to pick for the training set (default: 0 = no mirroring)',
                   type=int, default=0)
